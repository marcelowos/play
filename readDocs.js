...
MongoClient.connect(connectionURL, {useNewUrlParser: true}, (error, client) => {
    ...
    //find one doc. provide the collection name. 
    //take 2 arguments > object search criteria. callback that gets called when operation is completed
    // find returns the first item that matchs the search criteria
    // db.collection('users').findOne( { name: 'supr man' }, (error, user) => {
    //     if(error){
    //         console.log('erro')
    //     }
    //         console.log(user)
    ...
    db.collection('users').find( {job: 'super hero'}).toArray( (error, users) => {
        console.log(users)
    })
})
//
